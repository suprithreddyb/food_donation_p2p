...
export default function RequestsComponent( { setNewDonation } ) {
  ...
  const getData = async () => {
    const data = await service.patch( `order/orders?type=request&sortBy=${filters.sortBy}&sortType=${filters.sortType}`, userBody );
    return data.data;
  }
        ...
        <div>
          ...
          style={{
            ...
            <Group justify="space-between">
              ...
              <Button variant="filled" size="xs" radius="lg" onClick = {() => apply( order._id, order.dist)}>Donate</Button>
            </Group>
          </Card>
        </div> 
        )
